...
public class SolitaireDisplay extends JComponent implements MouseListener
{
    ...
    public void paintComponent(Graphics g)
    {
        ...
        if (selectedRow == 0)
        {
            ...
            if(selectedCol == 4)
                drawBorder(g, (CARD_WIDTH + SPACING) * 3 + SPACING, SPACING);
            if(selectedCol == 5)
                drawBorder(g, (CARD_WIDTH + SPACING) * 4 + SPACING, SPACING);
            if(selectedCol == 6)
                drawBorder(g, (CARD_WIDTH + SPACING) * 5 + SPACING, SPACING);
            if(selectedCol == 7)
                drawBorder(g, (CARD_WIDTH + SPACING) * 6 + SPACING, SPACING);
        }
    ...
    /** Draws a reset button to the right of the cards 
     * @postcondition a reset button is drawn to the right of the cards 
     * @param g a Graphics object to draw the imagine 
     * 
     */
    private void drawReset(Graphics g)
    {
        String fileName = "cards/resetsolitaire.jpg";
        Image image = new ImageIcon(fileName).getImage(); 
        g.drawImage(image, SPACING + (CARD_WIDTH + SPACING) * 7, SPACING, CARD_WIDTH, CARD_HEIGHT, null);
    }
    ...
    {
        return (selectedRow == 0 && (selectedCol == 4 || selectedCol==5 || selectedCol==6 
                || selectedCol==7));
    }

    /** determines which foundation is selected, if one is 
     * @return the index of the foundation that is selected, if one is 
     *          -1 if a foundation is not selected 
     * 
     */
    public int selectedFoundation()
    {
        if(isFoundationSelected())
            return selectedCol-4;
        return -1; 
    }

    /** selects a given foundation 
     *  @postcondition the given foundation is selected; the variable selectedRow is set to 0 
     *                     and selectedCol is set to the index of the foundation plus four 
